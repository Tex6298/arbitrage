...
DATASET_SPECS: Tuple[DatasetSpec, ...] = (
    ...
    DatasetSpec(
        ...
        status="implemented_first_pass",
        note="This is the first actual opportunity layer. It keeps route gating separate from source-magnitude selection, distinguishes current connector tightness from already-known upcoming connector restrictions, preserves explicit reviewed-versus-proxy internal-transfer lineage, and now has a switchable optional upstream market-state join for real route-level forward, day-ahead, or intraday price-state inputs when they exist.",
    ),
    DatasetSpec(
    ...
    DatasetSpec(
        ...
        status="implemented_first_pass",
        note="First pass now supports both a grouped-mean baseline and a stronger potential-ratio baseline over fact_curtailment_opportunity_hourly, and it is horizonized at explicit forecast offsets with forecast-origin timestamps and as-of feature lineage. The current v2 model can now prefer a real upstream market-state feed when present, and otherwise falls back to targeted R2_netback_GB_NL_DE_PL market-state features derived from the as-of route score, including raw route-price state and delta buckets, route-price transition and persistence buckets, as-of connector ITL state, internal-transfer gate state, and multi-hour connector/internal state paths. This table remains the backtest audit trail and should never be recomputed silently.",
    ),
    DatasetSpec(
