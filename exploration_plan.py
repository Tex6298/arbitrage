...
DATASET_SPECS: Tuple[DatasetSpec, ...] = (
    ...
    DatasetSpec(
        ...
        status="implemented_first_pass",
        note="First pass now supports both a grouped-mean baseline and a stronger potential-ratio baseline over fact_curtailment_opportunity_hourly, and it is horizonized at explicit forecast offsets with forecast-origin timestamps and as-of feature lineage. The current v2 model also keeps targeted route-state persistence and transition features for R2_netback_GB_NL_DE_PL, including as-of connector ITL state, internal-transfer gate state, transition-state lineage, and persistence buckets. This table remains the backtest audit trail and should never be recomputed silently.",
    ),
    DatasetSpec(
...
BACKTEST_SPECS: Tuple[BacktestSpec, ...] = (
    BacktestSpec(
        ...
        slice_dimensions="Cluster, route, delivery tier, notice market state, hour of day",
        acceptance_gate="Prediction basis and training-sample lineage must remain explicit so stronger models can be compared against the same audit trail.",
    ),
    BacktestSpec(
